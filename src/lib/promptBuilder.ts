...
import { formatDate, getNextElection } from "@/lib/deadlineUtils";

/**
 * The main ballot research prompt text (from docs/BALLOT_PROMPT.md, "The Prompt" section).
 * This is the static portion that gets combined with the state-specific context.
 */
...
This is what I bring to the polls. It should fit on a single printed page.

### Output B: My Voter Profile

This is my decision-making profile that I save for future elections.
...
/**
 * Builds the state-specific context block to append after the main prompt.
 */
export function buildContextBlock(
  ...
  const electionTypeLine = nextElection
  ...
  const mailDeadline = reg.byMail.deadline
    ? `By mail by ${formatDate(reg.byMail.deadline)}${reg.byMail.sincePostmarked ? " (postmark date)" : " (received date)"}`
    : "Mail registration not available";
  const inPersonDeadline = reg.inPerson.deadline
    ? `In person by ${formatDate(reg.inPerson.deadline)}`
    : "In-person registration info unavailable";

  const sameDayNote = reg.sameDayRegistration
    ? " · Same-day registration available"
    : "";

  const registrationLine = `**Registration deadlines:** ${onlineDeadline}; ${mailDeadline}; ${inPersonDeadline}${sameDayNote}`;

  // Early voting
  const ev = stateData.earlyVoting;
  const earlyVotingLine =
    ev.available && ev.startDate && ev.endDate
      ? `**Early voting:** ${formatDate(ev.startDate)} through ${formatDate(ev.endDate)}${ev.notes ? ` (${ev.notes})` : ""}`
      : `**Early voting:** Not available — absentee voting only`;

  // Voter ID
  const vr = stateData.votingRules;
  const voterIdLine = vr.idRequired
    ? `**Voter ID:** Required. Accepted IDs: ${vr.acceptedIds.slice(0, 3).join(", ")}${vr.acceptedIds.length > 3 ? ", and others" : ""}`
    : `**Voter ID:** Not required`;

  // Phones at polls
  const phonesLine = `**Phones at polls:** ${vr.phonesAtPollsDetail}`;

  // Resources
  const resources = stateData.resources;
  const sampleBallotLine = `**My sample ballot:** ${resources.sampleBallotLookup}`;
  const countyOffice = `**My county election office:** ${resources.countyElectionLookup}`;
...
/**
 * Builds the full prompt: main prompt + state-specific context block.
 */
export function buildFullPrompt(stateData: StateData, zipCode: string): string {
  const contextBlock = buildContextBlock(stateData, zipCode);
  return `${MAIN_PROMPT}\n\n---\n\n${contextBlock}`;
}

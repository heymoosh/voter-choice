...
import { getNextElection, getDeadlineInfo, formatDate } from "./election-data";

// The main prompt text extracted from docs/BALLOT_PROMPT.md
// Starting from "You are a nonpartisan civic research assistant..."
...
/**
 * Build the election type label for display in the prompt.
 */
...
/**
 * Generate the pre-filled context block for a given state and zip code.
 */
function generateContextBlock(
  ...
  const electionLine = election
    ...
    : `**Election:** No upcoming election found — check ${state.resources.stateElectionWebsite}`;

  const electionTypeLine = election
    ? `**Election type:** ${buildElectionTypeLabel(election)}`
    : null;

  const onlineDeadline = getDeadlineInfo(reg.online.available ? reg.online.deadline : null);
  const mailDeadline = getDeadlineInfo(reg.byMail.deadline);
  const inPersonDeadline = getDeadlineInfo(reg.inPerson.deadline);

  const onlineRegLine = reg.online.available && reg.online.deadline
    ? `Online by ${formatDate(reg.online.deadline)} (${onlineDeadline.label})`
    : "Not available online";

  const mailPostmark = reg.byMail.sincePostmarked ? "postmark date" : "received by";
  const mailRegLine = reg.byMail.deadline
    ? `By mail by ${formatDate(reg.byMail.deadline)} — ${mailPostmark} (${mailDeadline.label})`
    : "N/A";

  const inPersonRegLine = reg.inPerson.deadline
    ? `In person by ${formatDate(reg.inPerson.deadline)} (${inPersonDeadline.label})`
    : "N/A";

  const sameDayLine = reg.sameDayRegistration
    ? "Same-day registration available at the polls."
    : "No same-day registration.";

  const earlyVotingLine = state.earlyVoting.available && state.earlyVoting.startDate && state.earlyVoting.endDate
    ? `${formatDate(state.earlyVoting.startDate)} through ${formatDate(state.earlyVoting.endDate)}${state.earlyVoting.notes ? ` (${state.earlyVoting.notes})` : ""}`
  ...
  const lines = [
    `Hi! I'm voting in **${state.stateName}**. My zip code is **${zipCode}**.`,
    "",
    "Here's what I know about my upcoming election:",
    `- ${electionLine}`,
    electionTypeLine ? `- ${electionTypeLine}` : null,
    `- **Registration deadlines:** ${onlineRegLine}; ${mailRegLine}; ${inPersonRegLine}. ${sameDayLine}`,
...
/**
 * Generate the full customized prompt for a given state and zip code.
 */
export function generatePrompt(
  state: StateData,
  zipCode: string,
): GeneratedPrompt {
  const election = getNextElection(state);
  const contextBlock = generateContextBlock(state, zipCode, election);
  const fullText = `${MAIN_PROMPT}\n\n---\n\n${contextBlock}`;

...
- `.ai/work-packets/tdd-phase-2-mutation-testing.md` — TDD Phase 2: Stryker setup, scoped mutation testing on high-stakes paths, mutation-score threshold, CI integration. Status: SHIPPED 2026-05-20 (commit `d0f38ac`) — configuration verified locally; CI run + gate behavior NOT yet validated end-to-end (pending Phase 2a).
- `.ai/work-packets/tdd-phase-2a-end-to-end-validation.md` — TDD Phase 2a: end-to-end validation that the test + mutation gates actually block deliberately broken PRs and allow clean ones. Four PR cycles against `launch/production` (3 should-block + 1 should-pass). The should-pass PR strengthens a survived-mutant test via the Willison ritual. Status: ready. Prerequisite to closing out the TDD rollout and starting redesign work.

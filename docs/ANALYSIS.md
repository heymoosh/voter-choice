...
**The most structurally important finding:** BMAD was the only framework whose workflow asks "what _kind_ of test does this deliverable need?" — not just "are tests present?" BMAD's story-level Definition of Done explicitly gates on e2e tests for any story that describes a user flow. The other four frameworks ask only whether unit tests exist. This single workflow design decision is the entire reason BMAD's e2e suite grew with the feature (42→62) while every other framework's stayed flat. It's a gap in framework design, not a gap in Claude Code's capability — and it's directly observable in the data.

**Caveats:** This experiment has three structural limitations that must be acknowledged. First, all runs were autonomous (no human operator input), which is a realistic scenario but means qualitative "feel" data is absent. Second, Phase 1 runs 1–4 were invalidated and re-run after discovering frameworks were not being invoked (Learning 001) — the final datasets are from corrected runs with proper enforcement.

Third — and most important for how these numbers should be read — **every run in this analysis was a "rubric-visible" run**. The scoring rubric (metric thresholds, the 10-check auto-findings table, the data completeness gate, tool names like ESLint, Vitest, jscpd, Lighthouse, Playwright) was embedded in `/start.md` and in `scripts/measure.mjs` / `scripts/analyze-adherence.mjs`, all of which sat inside the repo mount that the workflow container could freely read. Claude Code reads the entire `/start.md` into context before beginning a build, so in every Phase 1 and Phase 2 run the workflow saw exactly which metrics and thresholds it would be judged on before writing any code. That is a metric-gaming vector: a sufficiently capable agent can optimize toward the disclosed numbers without actually producing code that's easier to maintain. See `docs/LEARNINGS.md` → Learning 009 for the full post-mortem and the scoring-isolation fix (tmpfs mask on `/workspace/scoring`, Hermes-side scoring from a separate `main` worktree, per-branch workflow files stripped of any rubric names, rubric consolidated at `scoring/auto-findings-rubric.md`). The numeric rankings in this document should therefore be treated as **rubric-visible baseline data**. Any future "clean-run" comparisons — done on new `run4/*` branches created after the isolation fix, with Hermes applying the rubric from the host side — must be labeled separately and contrasted against these numbers, not merged into a single leaderboard. Differences between the rubric-visible and clean-run numbers are themselves a finding about how much gaming was happening.
...
> - **Integration Tests:** Integration tests added/updated for component interactions _when story requirements demand them_
> - **End-to-End Tests:** E2e tests created for critical user flows _when story requirements specify them_
...
Spec Kit and Superpowers have _stronger_ unit test discipline than BMAD (91.6% and 89.9% line coverage vs 37.75%). But neither framework's workflow asks "does this feature need an e2e test?" That question doesn't exist anywhere in their implementation cycles. The result: a new user-facing feature (Spanish language toggle) ships with 87 new unit tests (Spec Kit) but zero new e2e tests. The feature works, the unit tests pass, and the e2e gap is invisible until a regression breaks the end-to-end flow.

...
| `PAC_TRANSPARENCY_ENABLED`        | confirmed     | Part 6 money-transparency blocks on seat cards: "Top PACs and their sponsors" (6a) + "Outside spending about this race" (6b)                         | Set `PAC_TRANSPARENCY_ENABLED=true` (exactly `true`) in Vercel prod env, then redeploy                                                                                                                | `src/lib/server/pac-transparency-flag.ts` |
...
- `PAC_TRANSPARENCY_ENABLED`: blocked on data, not code. Sponsor attributions
  are auto-classified from the FEC's `CONNECTED_ORG` field until a human
  curation pass marks them `verified`/`rejected`, and the 6b independent-
  expenditure ingest has only had a dry-run. Flip after Muxin's live 6b run +
  a curation pass (see `docs/DONOR_FRAMING_AND_ACCOUNTABILITY_PLAN.md`
  Part 6).

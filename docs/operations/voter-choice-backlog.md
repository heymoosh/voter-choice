...
- PRIORITY LOCK (Muxin, 2026-07-13): This is the first and only backlog priority. Do not begin or continue any other backlog implementation until this card is finished. The immediate next work on this card is follow-up planning, not implementation.
...
- PROPOSED PLAN (requires follow-up planning sessions before implementation):
...
- REQUIRED VALIDATION / SANITY CHECKS (also require follow-up planning):
...
- FOLLOW-UP PLANNING REQUIRED (Muxin, 2026-07-13): This card is intentionally not properly scoped for implementation yet. Every request in the verbatim block below requires one or more follow-up planning sessions. Do not treat the prose above as authorization to skip those sessions or start coding immediately.
...
- NEXT ACTION: Conduct dedicated follow-up planning sessions for the additional requirements above, then update this same card with a fully scoped task breakdown, source-coverage matrix, refresh/freshness design, database/app integration design, validation contract, rollout/rollback plan, and executable acceptance criteria. Do not plan those items in this backlog-edit session.
- GOAL_CONDITION (provisional until follow-up planning replaces it): `npm run verify:congressional-rosters -- --year 2026` passes with every expected federal contest mapped to either a complete official qualified roster for the exact election stage or an explicit `official_roster_not_yet_published` state; the address-to-upcoming-race app flow shows every qualified candidate from the latest verified complete official snapshot; and zero FEC-only, filed-only, defeated, withdrawn, or disqualified candidates are selectable.
- STATUS: To Do
- DECISION: #1 PRIORITY / QUEUE LOCK — follow-up planning sessions first, then implementation. Nothing else in the backlog may be worked until this card is finished.

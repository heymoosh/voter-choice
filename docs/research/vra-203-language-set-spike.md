...
2. Among the largest by both jurisdiction reach and covered population — they match 4 of LA County's own 5 non-Spanish languages (the single most multilingual jurisdiction in the country), and independently rank among the largest Asian languages by LEP population nationally: Chinese ~1.6M LEP speakers (6.4% of the national LEP population), Vietnamese ~850K (3.4%), Korean ~630K (2.5%) — [Migration Policy Institute, search-synthesized from their LEP data hub](https://www.migrationpolicy.org/programs/data-hub/charts/top-10-languages-spoken-limited-english-proficient-us-residents-and-lep). **Verification follow-up (2026-07-10, card `4268c35d`): still UNCONFIRMED against the primary chart.** The interactive chart itself, and every other page on migrationpolicy.org tried (article page, factsheet PDFs), returns HTTP 403 to every automated fetch tool available in this session (`WebFetch` direct, an archive.org mirror), and no live browser tool was available to do the literal manual visit the card asked for — this remains an open action item for an actual human browser session before external publication. A second, independently-run search did surface a specific-figure citation with the same ranking but different point values (Chinese 1.8M/7%, Vietnamese 867K/3%, Korean 592K/2% — source: MPI's own site, quoting what reads as a more recent ACS vintage than the ~1.6M/850K/630K figures above), which corroborates the *ranking and order of magnitude* but not the exact numbers — the two searches likely landed on different ACS-vintage snapshots of MPI's auto-updating chart. **Net effect: the Tier-1 verdict (Chinese/Vietnamese/Korean/Tagalog) is unaffected** — both searches agree they're among the largest Asian LEP populations nationally — but do not cite either set of exact point figures externally without first pulling current numbers directly from the live chart. Tagalog's specific national LEP population figure wasn't independently pinned in this spike (labeled uncertain) but it is one of Hawaii's top 3 LEP languages and one of the 8 nationally-triggered §203 Asian groups.

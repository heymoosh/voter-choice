...
**Run 5, Phase 1 — Vanilla workflow.** You are already on `run5/vanilla` — do NOT switch branches. Build the full production ballot research tool on this branch per `docs/PROJECT_SPEC.md` (v2.0). No framework — use default Claude Code approach only (no plugins, no slash commands beyond `/start`). Clean environment first (`rm -rf node_modules .next coverage && npm install`), build autonomously using the spec as the sole source of truth, then run `npm run measure`, tag `vanilla-run5-phase1-complete`, push, and update this log.

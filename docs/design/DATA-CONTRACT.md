...
canvas rule "close each data gap before building its UI" has a reference. Authored from
`main` + the reps-first work landing now. Statuses: **SHIPPED** (on main),
**IN PR** (built, awaiting review), **PLANNED** (decided, not built), **DOES NOT EXIST**.
...
Input: `{ address }` (street address; a zip alone cannot resolve a House district).
...
| `challengers[].topIssues` | **IN PR** — top-3 issues from the promise ledger, see §4 | absent = no promise corpus for that candidate |

## 2. POST /api/race-data — per-seat depth (SHIPPED; issue-optional behavior IN PR)
...
Reps-first default view is therefore **facts-only**: money facts, attendance,
...
## 4. Promise ledger — candidate issues & plans (IN PR)

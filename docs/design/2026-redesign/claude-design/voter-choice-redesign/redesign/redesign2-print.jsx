      ...
      <div className="print-wrap">
        ...
        <div className="print-sheet">
          <header className="ph-head">
            <div className="l">
              My Scorecard · {new Date(STATE_ELECTION_DATA.elections[0].date + "T00:00:00").toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
          ...
          <div className="voter-meta">
            ...
            <div className="cell"><div className="k">Your districts</div><div className="v" style={{ fontSize: "12px" }}>U.S. House TX-21 · TX Senate SD-14 · TX House HD-47</div></div>
            <div className="cell cell-bring">
              <div className="k">Bring (any one)</div>
              <ul className="v print-id-list">
                {STATE_ELECTION_DATA.votingRules.acceptedIds.map((id) => <li key={id}>{id}</li>)}
              </ul>
            </div>
            <div className="cell"><div className="k">Early voting</div><div className="v">
              {new Date(STATE_ELECTION_DATA.earlyVoting.startDate + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })} – {new Date(STATE_ELECTION_DATA.earlyVoting.endDate + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })}
            </div></div>
          </div>
          ...
          <div className="ballot-list">
                ...
                {ss.map((s) => {
                  ...
                  return (
                    <div className="br checked" key={s.id}>
                      ...
                      <div>
                        <div className="race-name">{s.office} · {s.districtLabel}</div>
                        <div className="pick-name">
                          {s.candidate.name}
                          <span className={"party verdict-print " + v}>{v === "keep" ? "WORTH KEEPING" : "TIME TO REPLACE"}</span>
                        </div>
                        <div className="my-note">
                          {frac ? frac + " · " : ""}{s.nextElection.label}
                        </div>
                      </div>
                    </div>
                  );
            ...
            {unreviewed.length > 0 && (
              <div className="ballot-group">
                <div className="gtitle" style={{ color: "var(--ink-3)" }}>Not yet reviewed</div>
                {unreviewed.map((s) => (
            ...
            <div className="ballot-group" style={{ marginBottom: 0 }}>
              ...
              <div style={{ fontSize: 13, color: "var(--ink-2)", lineHeight: 1.6 }}>
                {USER_ISSUES2.map((iss, i) => (
                  <div key={iss.canonicalIssue}>{i + 1}. {iss.interpretation} <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--ink-3)" }}>({iss.level === "both" ? "federal + state" : iss.level})</span></div>
                ))}
              </div>
            </div>
          </div>

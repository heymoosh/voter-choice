...
async function main() {
  ...
  const dis = JSON.parse(
    readFileSync(`${resolve("scripts/ingest/_gold-batches")}/_gold-disagreements.json`, "utf8"),
  ) as Array<{ issue: string; bill_id: string; oracle: string; pole_v1: string; kind: string; agreement: number }>;
  const fixes = dis.filter((d) => d.kind === "INVERSION");
  console.log(`${fixes.length} INVERSION correction(s) to apply (oracle direction wins):`);
  for (const f of fixes)
    console.log(`  [${f.issue}] ${f.bill_id}  ${f.pole_v1} → ${f.oracle}  (juror agreement ${f.agreement}/3)`);
  ...
  for (const f of fixes) {
    const before = (await sql`SELECT pole_stance FROM issue_tags_pole_v1 WHERE bill_id=${f.bill_id} AND canonical_issue=${f.issue}`) as Array<{ pole_stance: string }>;
    await sql`
    ...
    const after = (await sql`SELECT pole_stance FROM issue_tags_pole_v1 WHERE bill_id=${f.bill_id} AND canonical_issue=${f.issue}`) as Array<{ pole_stance: string }>;
    console.log(`  applied [${f.issue}] ${f.bill_id}: ${before[0]?.pole_stance} → ${after[0]?.pole_stance}`);
  }
  console.log(`\nDONE: ${fixes.length} correction(s) written (source_run='gold-adjudication-1').`);
}

main().catch((e) => { console.error("APPLY-FIX FAILED:", e.message); process.exit(1); });

...
async function main() {
  ...
  if (!issue)
    throw new Error(
      "usage: _gold-sample-more.ts <issue> [inFavorQ opposedQ noScoreQ]",
    );
  const Q = {
  ...
  const used = manifest
    ...
    .map((m) => Number(m.batchId.split("-").pop()));
  ...
  writeFileSync(
    `${BASE}/_manifest.json`,
    JSON.stringify([...manifest, ...newBatches], null, 2),
  );
  ...
  console.log(
    `${issue}: +${picked.length} NEW bills [pv ${byStance.in_favor || 0}/${byStance.opposed || 0}/${byStance.no_score || 0}] ` +
      `(excluded ${existingIds.length} already-sampled) → ${newBatches.length} new batch(es): ${newBatches.map((b) => b.batchId).join(", ")}`,
  );
  console.log(
    `Run the oracle panel on the new batches, then re-assemble + re-validate.`,
  );
  console.log(
    JSON.stringify(
      newBatches.map((b) => ({
...
main().catch((e) => {
  console.error("SAMPLE-MORE FAILED:", e.message);
  process.exit(1);
});

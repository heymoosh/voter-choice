...
async function main() {
  ...
  const consensus = JSON.parse(
    readFileSync(`${BASE}/_gold-consensus.json`, "utf8"),
  ) as Record<string, { oracle: string; agreement: number; jurors: string[] }>;
  const pv = JSON.parse(
    readFileSync(`${BASE}/_gold-pv.json`, "utf8"),
  ) as Record<string, { pole_stance: string; confidence: string | null }>;
  if (!existsSync(`${BASE}/_manifest.json`))
    throw new Error("run _gold-sample.ts first");
  ...
  for (const r of oldRows)
    oldStance[`${r.bill_id}|${r.canonical_issue}`] = r.stance_lens;
  ...
  const results: Array<{
    ...
    rate: number;
    pass: boolean;
    pvAbstainOnConf: number;
    oracleAbstainPvConf: number;
    oldN: number;
    oldInv: number;
  }> = [];
      ...
      if (CONF.has(o) && CONF.has(p)) {
        ...
        if (o !== p) {
          ...
          disagreements.push({
            ...
            pole_v1: p,
            kind: "INVERSION",
            agreement: consensus[key].agreement,
            jurors: consensus[key].jurors,
            pv_conf: pv[key].confidence,
            old: oldStance[key] ?? null,
            title: billMeta[key]?.title,
            summary: (billMeta[key]?.summary || "").slice(0, 600),
          });
        }
        ...
        disagreements.push({
          ...
          oracle: o,
          pole_v1: p,
          kind: "PV_ABSTAINED_ON_CONFIDENT",
          agreement: consensus[key].agreement,
          jurors: consensus[key].jurors,
          old: oldStance[key] ?? null,
          title: billMeta[key]?.title,
        ...
        disagreements.push({
          ...
          oracle: o,
          pole_v1: p,
          kind: "ORACLE_ABSTAINED_PV_CONFIDENT",
          agreement: consensus[key].agreement,
          jurors: consensus[key].jurors,
          pv_conf: pv[key].confidence,
          title: billMeta[key]?.title,
  ...
  for (const issue of ISSUES) {
    ...
    const after = (
      ...
      WHERE p.canonical_issue = ${issue} AND p.pole_stance IN ('in_favor','opposed')
        AND v.vote_date >= ${cutoff}`
    )[0].n;
    coverage[issue] = { before, after };
  }

  writeFileSync(
    `${BASE}/_gold-disagreements.json`,
    JSON.stringify(disagreements, null, 2),
  );

  // ---- report ----
  console.log(
    `=== CUTOVER GATE — per-issue inversion (threshold ${(threshold * 100).toFixed(1)}%) ===`,
  );
  console.log(
    `(gate = inversion ≤ threshold; improvement-over-old shown as DIAGNOSTIC only)\n`,
  );
  console.log(
    `issue                 n(conf-both)  inv   rate    GATE   | old: inv/n  | pv→no_score on conf | oracle→no_score on pv-conf`,
  );
  ...
  for (const issue of ISSUES) {
    ...
    const delta = c.before
      ? Math.round(((c.after - c.before) / c.before) * 100)
      : 0;
    console.log(
      `${issue.padEnd(20)} ${String(c.before).padStart(8)}  ${String(c.after).padStart(12)}        ${delta}%`,
    );
  }
  ...
  console.log(
    `Disagreements for adjudication: ${disagreements.length} → _gold-disagreements.json`,
  );
}

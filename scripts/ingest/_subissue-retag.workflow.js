export const meta = {
  ...
  description:
    "Assign a healthcare sub_issue facet to each healthcare_affordability bill on the Neon alignment branch (Sonnet taggers, schema-validated). Sub-issues INHERIT the parent pole — no new direction.",
  phases: [
    {
      title: "SubTag",
...
for (let attempt = 1; attempt <= 3 && pending.length > 0; attempt++) {
  log(
    `SubTag attempt ${attempt}: ${pending.length} batch(es), ${done.length} done`,
  );
...
log(
  `Sub-tagging complete: ${done.length} batches, ${totalWritten} entries written; ${pending.length} still failing`,
);

/**
 * scripts/ingest/id-donors.ts
 *
 * Idaho donor ingest from Idaho Sunshine Campaign Finance System.
 * Source: https://sunshine.voteidaho.gov/public/cf/downloads
 * API: POST https://api-sunshine.voteidaho.gov/api/ExportData/GetExportPublicDownloadData
 *
 * Download command (no auth required):
 *   curl -s -X POST \
 *     "https://api-sunshine.voteidaho.gov/api/ExportData/GetExportPublicDownloadData" \
 *     -H "Content-Type: application/json" -H "Referer: https://sunshine.voteidaho.gov/" \
 *     -H "User-Agent: Mozilla/5.0" \
 *     -d '{"transactionTypeCode":"TCON","type":"CSV","filingYear":"2024","openInNewTab":false}' \
 *     -o /tmp/ID_contributions_2024.csv
 *
 * CSV format: same platform as VT (same API vendor).
 *   Filing Entity Name ("Last, First"), Registration Type (Candidate|...),
 *   Contributor Type (Person|Company|Political Committee|Self|Central Committee|...),
 *   Contributor Last Name, Contributor First Name, Contributor Company Name,
 *   Transaction Amount
 *
 * File encoding: latin-1 (has non-UTF-8 characters)
 *
 * Usage:
 *   DATABASE_URL=<neon> npx tsx scripts/ingest/id-donors.ts [--dry-run]
 *   DATABASE_URL=<neon> npx tsx scripts/ingest/id-donors.ts --file /tmp/ID_contributions_2024.csv
 *
...
const DEFAULT_FILE = "/tmp/ID_contributions_2024.csv";
const SOURCE = "id_sunshine_bulk";
const SOURCE_URL = "https://sunshine.voteidaho.gov/public/cf/downloads";
const ELECTION_CYCLE = "2024";
const DOWNLOAD_CMD = `curl -s -X POST \\
  "https://api-sunshine.voteidaho.gov/api/ExportData/GetExportPublicDownloadData" \\
  -H "Content-Type: application/json" \\
  -H "Referer: https://sunshine.voteidaho.gov/" \\
  -H "User-Agent: Mozilla/5.0" \\
  -d '{"transactionTypeCode":"TCON","type":"CSV","filingYear":"2024","openInNewTab":false}' \\
  -o ${DEFAULT_FILE}`;
...
async function main() {
  ...
  if (!fs.existsSync(filePath)) {
    console.error(`[id] file not found: ${filePath}`);
  ...
  for (const c of [...idHouse, ...idSenate]) {
  ...
  for await (const line of rl) {
    ...
    const amountStr = (row["Transaction Amount"] ?? "")
      .replace(/[$,="]/g, "")
      .trim();
    ...
    if (contribType === "Self") {
      bucket = "Self-funded";
    } else if (contribType === "Central Committee") {
    ...
    if (rowsProcessed % 5000 === 0) {
      console.log(`[id] processed=${rowsProcessed}`);
    }
  }
  ...
  console.log(
    `[id] processed=${rowsProcessed} skipped=${rowsSkipped} ` +
  ...
  if (isDryRun) {
    ...
    for (const [key, amt] of sample) {
      ...
      console.log(
        `[id] [dry-run] candidate=${cid} bucket=${bucket} amount=${amt.toFixed(2)}`,
      );
    }
    return;
  }
  ...
  for (const [aggKey, amount] of agg) {
    ...
    await db
      ...
      .values({
        ...
        rawMetadata: { idFilingName: candidateNames.get(candidateId) ?? "" },
        insertedAt: new Date(),
      })
      .onConflictDoUpdate({
        target: [
  ...
  console.log(`[id] done upserted=${upserted}`);
}

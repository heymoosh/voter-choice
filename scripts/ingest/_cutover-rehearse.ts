...
function fourYearsAgo(): string {
  const d = new Date(); d.setFullYear(d.getFullYear() - 4); return d.toISOString().slice(0, 10);
}
...
async function main() {
  ...
  try {
    ...
    for (const issue of ISSUES) {
      const pv = (await sql`SELECT bill_id, pole_stance, tagger_confidence FROM issue_tags_pole_v1 WHERE canonical_issue=${issue}`) as Array<{ bill_id: string; pole_stance: string; tagger_confidence: string }>;
      const confident = pv.filter((r) => r.pole_stance === "in_favor" || r.pole_stance === "opposed");
      const noScoreIds = pv.filter((r) => r.pole_stance === "no_score").map((r) => r.bill_id);
      for (let i = 0; i < confident.length; i += CHUNK) {
        ...
        await sql.query(
          ...
          [c.map((r) => r.bill_id), c.map(() => issue), c.map((r) => r.pole_stance),
           c.map(() => TAGGER_VERSION), c.map((r) => CONF_MAP[r.tagger_confidence] ?? "0.400")],
        );
      }
      for (let i = 0; i < noScoreIds.length; i += CHUNK) {
        const ids = noScoreIds.slice(i, i + CHUNK);
        await sql.query(`DELETE FROM ${T} WHERE canonical_issue=$1 AND bill_id = ANY($2)`, [issue, ids]);
      }
    }

    // ---- verify per-issue counts ----
    console.log("=== REHEARSAL post-migration counts (clone) ===");
    console.log("issue                 after  pv_confident  match  | old(issue_tags)");
    let ok = true;
    for (const issue of ISSUES) {
      const after = (await sql.query(`SELECT count(*)::int n FROM ${T} WHERE canonical_issue=$1`, [issue]))[0].n;
      const conf = (await sql`SELECT count(*)::int n FROM issue_tags_pole_v1 WHERE canonical_issue=${issue} AND pole_stance IN ('in_favor','opposed')`)[0].n;
      const old = (await sql`SELECT count(*)::int n FROM issue_tags WHERE canonical_issue=${issue}`)[0].n;
      const m = after === conf;
      if (!m) ok = false;
      console.log(`${issue.padEnd(20)} ${String(after).padStart(5)}  ${String(conf).padStart(12)}  ${m ? "✓" : "✗"}      | ${old}`);
    }
    const leak = (await sql.query(`SELECT count(*)::int n FROM ${T} WHERE stance_lens='no_score'`))[0].n;
    const afterTotal = (await sql.query(`SELECT count(*)::int n FROM ${T}`))[0].n;
    console.log(`\nno_score leak in clone: ${leak} ${leak === 0 ? "✓" : "✗"}`);
    console.log(`clone total: ${before} → ${afterTotal} (Δ ${afterTotal - before})`);
    ...
    for (const issue of ["energy_grid", "election_integrity", "public_safety"]) {
      const cand = (await sql`
        SELECT v.candidate_id id FROM votes v JOIN issue_tags it ON it.bill_id=v.bill_id
        WHERE it.canonical_issue=${issue} AND v.vote_date>=${cutoff} LIMIT 1`) as Array<{ id: string }>;
      if (!cand.length) { console.log(`${issue}: no sample candidate`); continue; }
      const id = cand[0].id;
      const score = async (table: string) => {
        const rows = (await sql.query(
          `SELECT v.vote_cast vc, t.stance_lens sl FROM votes v
           ...
           WHERE v.candidate_id=$2 AND v.vote_date>=$3`, [issue, id, cutoff])) as Array<{ vc: string; sl: string }>;
        const s = rows.map((r) => computeVoteAlignment(r.vc, r.sl, "in_favor")).filter((a) => a !== "abstain");
        return `kept ${s.filter((a) => a === "with").length}/${s.length}`;
      };
      console.log(`  ${issue.padEnd(20)} cand ${id.slice(0, 22)}  before=${await score("issue_tags")}  after=${await score(T)}`);
    }
...
main().catch((e) => { console.error("REHEARSE FAILED:", e.message); process.exit(1); });

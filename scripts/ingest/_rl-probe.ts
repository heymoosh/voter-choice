...
async function main(){
  ...
  const res=await fetch("https://v3.openstates.org/jurisdictions?per_page=1",{headers:{"X-API-KEY":key,"user-agent":"vc-probe"}});
  console.log("status:",res.status);
  const h:Record<string,string>={};
  res.headers.forEach((v,k)=>{if(/rate|limit|retry|quota|remain/i.test(k))h[k]=v;});

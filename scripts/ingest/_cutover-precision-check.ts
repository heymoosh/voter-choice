...
async function main() {
  ...
  const dis = JSON.parse(readFileSync(`${BASE}/_gold-disagreements.json`, "utf8")) as Array<{
    issue: string; kind: string; pv_conf?: string;
  }>;

  console.log("=== (1) ORACLE-ABSTAIN bucket (pole_v1 confident, oracle no_score) by pole_v1 confidence ===");
  ...
  for (const i of ISSUES) {
    ...
    th += b.high; tm += b.medium; tl += b.low;
    if (t) console.log(`${i.padEnd(20)} ${String(b.high).padStart(4)} ${String(b.medium).padStart(4)} ${String(b.low).padStart(4)} ${String(t).padStart(6)}`);
  }
  console.log(`${"TOTAL".padEnd(20)} ${String(th).padStart(4)} ${String(tm).padStart(4)} ${String(tl).padStart(4)} ${String(th + tm + tl).padStart(6)}`);

  console.log("\n=== (2) UNMEASURED ship population: confident pole_v1 tags by summary presence ===");
  console.log("(gold sample was 100% with-summary; null-summary confident tags ship un-validated)");
  console.log("issue                 confident  withSummary  nullSummary  %null");
  ...
  for (const i of ISSUES) {
    const r = (await sql`
      ...
      WHERE p.canonical_issue = ${i} AND p.pole_stance IN ('in_favor','opposed')`)[0] as
      { total: number; withsum: number; nullsum: number };
    cAll += r.total; nAll += r.nullsum;
    const pctNull = r.total ? Math.round((r.nullsum / r.total) * 100) : 0;
    console.log(`${i.padEnd(20)} ${String(r.total).padStart(9)}  ${String(r.withsum).padStart(11)}  ${String(r.nullsum).padStart(11)}  ${pctNull}%`);
  }
  console.log(`${"TOTAL".padEnd(20)} ${String(cAll).padStart(9)}  ${String(cAll - nAll).padStart(11)}  ${String(nAll).padStart(11)}  ${Math.round((nAll / cAll) * 100)}%`);
}

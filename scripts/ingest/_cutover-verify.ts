...
async function main() {
  ...
  console.log("issue                 prod_now  pv_confident(expected if migrated)");
  for (const i of ALL_CONTESTED) {
    const now = (await sql`SELECT count(*)::int n FROM issue_tags WHERE canonical_issue=${i}`)[0].n;
    const pvConf = (await src`SELECT count(*)::int n FROM issue_tags_pole_v1 WHERE canonical_issue=${i} AND pole_stance IN ('in_favor','opposed')`)[0].n;
    console.log(`${i.padEnd(20)} ${String(now).padStart(8)}  ${pvConf}`);
  }

  // 2. no_score leak check
  const leak = (await sql`SELECT count(*)::int n FROM issue_tags WHERE stance_lens='no_score'`)[0].n;
  ...
  for (const c of cands) {
    const rows = (await sql`
      ...
      WHERE v.candidate_id = ${c.id} AND v.vote_date >= ${cutoff}`) as Array<{ vote_cast: string; stance_lens: string }>;
    const scored = rows
      .map((r) => computeVoteAlignment(r.vote_cast, r.stance_lens, stance))
      .filter((a) => a !== "abstain");
    const kept = scored.filter((a) => a === "with").length;
    console.log(`  candidate ${c.id.slice(0, 28).padEnd(28)} kept ${kept}/${scored.length}` +

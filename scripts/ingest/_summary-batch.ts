...
function main() {
  ...
  const lines = readFileSync(`${DIR}/_summary-text.jsonl`, "utf8")
    .split("\n")
    .filter(Boolean);
  ...
  for (const line of lines) {
    ...
    if (contested.length === 0) {
      ...
      for (const issue of contested)
        (buckets[issue] ??= []).push({ id: o.id, text });
    }
  }
  ...
  for (const [key, rows] of Object.entries(buckets)) {
    ...
    const flush = () => {
      ...
      writeFileSync(
        `${abs}/${batchId}.json`,
        JSON.stringify({ batchId, issue, bills: batch }),
      );
      manifest.push({
        batchId,
        issue,
        resultPath: `${abs}/_results/${batchId}.json`,
        count: batch.length,
      });
      batch = [];
      chars = 0;
    ...
    for (const r of rows) {
      if (batch.length >= MAX_BILLS || chars + r.text.length > MAX_CHARS)
        flush();
      batch.push(r);
      chars += r.text.length;
    }
  ...
  console.log(
    `batches: ${manifest.length} (contested ${contestedBatches}, summary-only ${summaryBatches})`,
  );
  console.log(`bill-issue pairs: ${billPairs}`);
  console.log("per-issue batch counts:");
  const byIssue: Record<string, number> = {};
  for (const m of manifest)
    byIssue[m.issue ?? "summary_only"] =
      (byIssue[m.issue ?? "summary_only"] ?? 0) + 1;

...
async function main() {
  ...
  if (!planPath)
    throw new Error("usage: _cutover-fire.ts <plan.json> [--fire]");
  ...
  for (const i of [...migrate, ...blank])
    if (!ALL_CONTESTED.includes(i))
      throw new Error(`plan references non-contested issue: ${i}`);
  const overlap = migrate.filter((i) => blank.includes(i));
  if (overlap.length)
    throw new Error(`issue in BOTH migrate and blank: ${overlap.join(", ")}`);

  const prodUrl = cutoverVar("CUTOVER_PROD_DATABASE_URL");
  if (!prodUrl)
    throw new Error(
      "CUTOVER_PROD_DATABASE_URL not set (env or .env.cutover) — required even for dry-run preflight",
    );
  const srcUrl = loadAlignmentUrl();
  if (hostOf(prodUrl) === hostOf(srcUrl))
    throw new Error(
      "PROD url host == alignment-branch host — refusing (point CUTOVER_PROD_DATABASE_URL at PRODUCTION)",
    );

  const prod = neon(prodUrl);
  const src = neon(srcUrl);
  console.log(
    `MODE: ${FIRE ? "🔥 FIRE (will mutate prod)" : "DRY-RUN (read-only preflight)"}`,
  );
  ...
  for (const issue of migrate) {
    const prodN = (
      await prod`SELECT count(*)::int n FROM issue_tags WHERE canonical_issue=${issue}`
    )[0].n;
    const pvRows =
      (await src`SELECT bill_id, pole_stance, tagger_confidence FROM issue_tags_pole_v1 WHERE canonical_issue=${issue}`) as Array<{
        bill_id: string;
        pole_stance: string;
    ...
    const prodIds =
      (await prod`SELECT bill_id FROM issue_tags WHERE canonical_issue=${issue}`) as Array<{
        bill_id: string;
      }>;
    ...
    if (!ok) blocked = true;
    console.log(
      `  ${issue.padEnd(20)} prod=${prodN} pv=${pvRows.length} (upsert ${conf}, delete ${noScore}) ` +
  ...
  if (blocked)
    throw new Error(
      "ABORT: key-set gap in a migrate issue (would leave stale/inverted rows). Investigate before firing.",
    );
  ...
  const exists = (
    await prod`SELECT to_regclass(${BACKUP_TABLE}) IS NOT NULL AS e`
  )[0].e;
  if (exists)
    throw new Error(
      `${BACKUP_TABLE} already exists — refusing to overwrite. Drop/rename it if this is a re-run.`,
    );
  await prod.query(
  ...
  const backupN = (
    await prod.query(`SELECT count(*)::int n FROM ${BACKUP_TABLE}`)
  )[0].n;
  const dump = (await prod.query(`SELECT * FROM ${BACKUP_TABLE}`)) as unknown[];
  writeFileSync(
    `scripts/ingest/_cutover-backup-${backupN}rows.json`,
    JSON.stringify(dump),
  );
  console.log(
  ...
  for (const issue of migrate) {
    ...
    const confident = pvRows.filter(
      (r) => r.pole_stance === "in_favor" || r.pole_stance === "opposed",
    );
    const noScoreIds = pvRows
    ...
    for (let i = 0; i < noScoreIds.length; i += CHUNK) {
      ...
      queries.push(
        prod`DELETE FROM issue_tags WHERE canonical_issue=${issue} AND bill_id = ANY(${ids})`,
      );
    }
    ...
    console.log(
      `  MIGRATED ${issue.padEnd(20)} upsert ${confident.length}, delete ${noScoreIds.length} → prod now ${after}`,
    );
  }
  for (const issue of blank) {
    await prod.transaction([
      prod`DELETE FROM issue_tags WHERE canonical_issue=${issue}`,
    ]);

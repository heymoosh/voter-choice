...
const toOcd = (id: string) =>
  id.replace(/^openstates-/, "").replace(/^ocd-bill-/, "ocd-bill/");
...
async function main() {
  ...
  const rows = (await sql`
    ...
    WHERE b.summary IS NULL AND b.source = 'openstates'
      AND EXISTS (SELECT 1 FROM issue_tags it WHERE it.bill_id = b.id)`) as Array<{
    id: string;
  }>;
  const map: Record<string, string> = {};
  for (const r of rows) map[toOcd(r.id)] = r.id;
  writeFileSync(
    "scripts/ingest/_pole-batches/_summary-idset.json",
    JSON.stringify(map),
  );
  console.log(
    `idset: ${rows.length} bills · distinct ocd ids: ${Object.keys(map).length}`,
  );

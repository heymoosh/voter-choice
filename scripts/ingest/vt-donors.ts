/**
 * scripts/ingest/vt-donors.ts
 *
 * Vermont donor ingest from the Vermont Campaign Finance System.
 * Source: https://campaignfinance.vermont.gov/public/cf/downloads
 * API: POST https://api.campaignfinance.vermont.gov/api/ExportData/GetExportPublicDownloadData
 *
 * Download command (no auth required):
 *   curl -s -X POST \
 *     "https://api.campaignfinance.vermont.gov/api/ExportData/GetExportPublicDownloadData" \
 *     -H "Content-Type: application/json" -H "Referer: https://campaignfinance.vermont.gov/" \
 *     -H "User-Agent: Mozilla/5.0" \
 *     -d '{"transactionTypeCode":"TCON","type":"CSV","filingYear":"2024","openInNewTab":false}' \
 *     -o /tmp/VT_contributions_2024.csv
 *
 * CSV key columns:
 *   Filing Entity Name (format: "LAST, FIRST [MIDDLE]"),
 *   Registration Type (Candidate|Political Action Committee|...),
 *   Contributor Type (Individual|Business/Group/Organization|PAC|Self|Political Party|...),
 *   Contributor Last Name (org name for non-individuals),
 *   Transaction Amount
 *
 * Usage:
 *   DATABASE_URL=<neon> npx tsx scripts/ingest/vt-donors.ts [--dry-run]
 *   DATABASE_URL=<neon> npx tsx scripts/ingest/vt-donors.ts --file /tmp/VT_contributions_2024.csv
 *
...
const DEFAULT_FILE = "/tmp/VT_contributions_2024.csv";
const SOURCE = "vt_cf_bulk";
const SOURCE_URL = "https://campaignfinance.vermont.gov/public/cf/downloads";
const ELECTION_CYCLE = "2024";
const DOWNLOAD_CMD = `curl -s -X POST \\
  "https://api.campaignfinance.vermont.gov/api/ExportData/GetExportPublicDownloadData" \\
  -H "Content-Type: application/json" \\
  -H "Referer: https://campaignfinance.vermont.gov/" \\
  -H "User-Agent: Mozilla/5.0" \\
  -d '{"transactionTypeCode":"TCON","type":"CSV","filingYear":"2024","openInNewTab":false}' \\
  -o ${DEFAULT_FILE}`;
...
/**
 * Parse VT filing entity name: "LAST, FIRST [MIDDLE]"
 */
function parseFilingEntityName(
  name: string,
...
async function main() {
  ...
  if (!fs.existsSync(filePath)) {
    console.error(`[vt] file not found: ${filePath}`);
  ...
  // Load VT state legislative candidates
  const vtHouse = (await db
    .select()
    .from(candidates)
    .where(
      sql`${candidates.jurisdiction} = 'state-VT-house'`,
    )) as DbCandidate[];
  const vtSenate = (await db
    .select()
    .from(candidates)
    .where(
  ...
  for (const c of [...vtHouse, ...vtSenate]) {
  ...
  // Aggregation: candidateId|cycle|bucket → amount
  ...
  for await (const line of rl) {
    ...
    // Only process Candidate rows
    const regType = row["Registration Type"]?.trim() ?? "";
    ...
    // Parse filing entity name → candidate name
    const filingName = row["Filing Entity Name"]?.trim() ?? "";
    const parsed = parseFilingEntityName(filingName);
    if (!parsed) {
      rowsSkipped++;
      continue;
    }
    ...
    const orgName = row["Contributor Last Name"]?.trim() ?? "";

    let bucket: DonorBucketLabel;

    if (contribType === "Self") {
      bucket = "Self-funded";
    } else if (contribType === "Political Party") {
      bucket = "Party committees";
    } else if (
      contribType === "Business/Group/Organization" ||
      contribType === "PAC" ||
      contribType === "Legislative Leadership PAC"
    ) {
      bucket = mapEmployerToBucket(orgName) ?? "Other";
    } else {
      // Individual, Candidate, empty (under-threshold withheld), etc.
    ...
    if (rowsProcessed % 5000 === 0) {
      console.log(`[vt] processed=${rowsProcessed}`);
    }
  }
  ...
  console.log(
    `[vt] processed=${rowsProcessed} skipped=${rowsSkipped} ` +
  ...
  if (isDryRun) {
    ...
    for (const [key, amt] of sample) {
      ...
      console.log(
        `[vt] [dry-run] candidate=${cid} bucket=${bucket} amount=${amt.toFixed(2)}`,
      );
    }
    return;
  }
  ...
  for (const [aggKey, amount] of agg) {
    ...
    await db
      ...
      .values({
        ...
        rawMetadata: { vtFilingName: candidateNames.get(candidateId) ?? "" },
        insertedAt: new Date(),
      })
      .onConflictDoUpdate({
        target: [
  ...
  console.log(`[vt] done upserted=${upserted}`);
}
...
if (isCliExecution()) {
  main().catch((e) => {
    console.error("[vt] error:", e instanceof Error ? e.message : e);
    process.exitCode = 1;
  });
}

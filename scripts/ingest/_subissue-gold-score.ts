...
function main() {
  ...
  const taggerLabels = JSON.parse(readFileSync(`${DIR}/_tagger-labels.json`, "utf8")) as Record<
    string,
    string
  >;
  ...
  for (const facet of FACETS) {
    ...
    for (let j = 1; j <= 3; j++) {
      ...
      if (!existsSync(f)) throw new Error(`missing juror file: ${f} — run the gold oracle first`);
      const data = JSON.parse(readFileSync(f, "utf8"));
      for (const t of data.tags) {
        (jurorVotes[t.bill_id] = jurorVotes[t.bill_id] || []).push(t.sub_issue ?? null);
      }
    }
  }
  ...
  const per: Record<
    string,
    { n: number; agree: number; contradict: number; unconfirmed: number; split: number }
  > = {};
  for (const f of FACETS) per[f] = { n: 0, agree: 0, contradict: 0, unconfirmed: 0, split: 0 };

  const contradictions: Array<{ bill_id: string; tagger: string; panel: string }> = [];
  ...
  if (contradictions.length) {
    ...
    if (contradictions.length > 40) console.log(`  …and ${contradictions.length - 40} more`);
  }
  if (failed.length) {

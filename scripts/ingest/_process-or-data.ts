...
function classifyOrestarContributor(
  ...
  if (/\(\d+\)/.test(contributor)) {
    ...
    if (
      upper.includes("DEMOCRATIC") ||
      upper.includes("REPUBLICAN") ||
      upper.includes("PARTY") ||
      upper.includes("CENTRAL COMMITTEE")
    ) {
...
async function main() {
  ...
  console.log(
    `[or-process] loaded ${raw.length} contributions from ${DATA_FILE}`,
  );
  ...
  const matchedCandidates = new Set(
    [...agg.keys()].map((k) => k.split("|")[0]),
  );
  console.log(
    `[or-process] matched=${matched} skipped=${skipped} candidates=${matchedCandidates.size} upsert_rows=${agg.size}`,
  );
  ...
  if (isDryRun) {
    for (const [key, amt] of [...agg.entries()].slice(0, 8)) {
      ...
      console.log(
        `  candidate=${cid} bucket=${bucket} amount=${amt.toFixed(2)}`,
      );
    }
    return;
  }
...
main().catch((e) => {
  console.error("[or-process] error:", e.message);
  process.exitCode = 1;
});

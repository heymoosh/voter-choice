...
function classifyContrib(
  ...
  if (/\bREPUBLICAN\b|\bDEMOCRAT(IC)?\b|\bGOP\b|\bPARTY\b/.test(combined))
...
async function navigateToSearch(page: Page, officeCode: string) {
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      await page.waitForSelector("#ddlViewerOptions", {
        timeout: 20000,
        state: "visible",
      });
      ...
      console.log(
        `[ks-cfr] navigateToSearch attempt ${attempt + 1} failed, retrying...`,
      );
      await sleep(5000);
    }
  }
...
async function getScheduleARows(
  page: Page,
): Promise<
  Array<{ name: string; occupation: string; employer: string; amount: number }>
> {
  const rows: Array<{
    name: string;
    occupation: string;
  ...
  while (true) {
    const pageRows = await page.evaluate(() => {
      return Array.from(document.querySelectorAll("table tr")).flatMap(
        (row) => {
          const cells = row.querySelectorAll("td");
          if (cells.length < 5) return [];
          const amtText =
            Array.from(cells).reverse()[0]?.textContent?.trim() ?? "";
    ...
    const hasNext = await page.evaluate(
      (pg) =>
        Array.from(document.querySelectorAll("a[href*='doPostBack']")).some(
          (l) => l.textContent?.trim() === String(pg + 1),
        ),
      pg,
    );
    if (!hasNext) break;
    await page.evaluate((pg) => {
      const l = Array.from(
        document.querySelectorAll("a[href*='doPostBack']"),
      ).find((a) => a.textContent?.trim() === String(pg + 1)) as
        | HTMLElement
        | undefined;
      l?.click();
    }, pg);
...
async function main() {
  ...
  function matchCandidate(
    portalName: string,
    chamber: string,
  ): (typeof dbCandidates)[0] | null {
    // Portal: "LASTNAME FIRSTNAME [MIDDLE]"
    const parts = norm(portalName).split(" ").filter(Boolean);
    if (!parts.length) return null;
    const lastName = parts[0]!;
    const firstName = parts[1] ?? "";
    const pool = (byLast.get(lastName) ?? []).filter((c) =>
      c.jurisdiction.includes(chamber),
    );
  ...
  try {
    for (const office of OFFICES) {
      ...
      while (true) {
        ...
        const candidates_on_page = await page.evaluate(() =>
          Array.from(document.querySelectorAll("a[href*='doPostBack']"))
            ...
            .map((l) => ({
              ...
              arg: l.getAttribute("href")?.match(/'([^']+)'/)?.[1] ?? "",
            })),
        );
        ...
        for (const c of candidates_on_page) {
          ...
          try {
            ...
            for (let p = 1; p < pageNum; p++) {
              await page.evaluate((p) => {
                const l = Array.from(
                  document.querySelectorAll("a[href*='doPostBack']"),
                ).find((a) => a.textContent?.trim() === String(p + 1)) as
                  | HTMLElement
                  | undefined;
                l?.click();
              }, p);
          ...
          const totalMatch = reportText.match(
            /TOTAL CONTRIBUTIONS AND OTHER RECEIPTS[^\$]*\$([\d,.]+)/i,
          );
          const totalAmt = totalMatch
          ...
          console.log(
            `[ks-cfr] [p${pageNum}] ${c.name} → $${totalAmt.toFixed(2)} → ${dbMatch.fullName}`,
          );

          // Navigate to Schedule A
          let contribRows: Array<{
            name: string;
            occupation: string;
            employer: string;
            amount: number;
            ...
            try {
              ...
              await page.evaluate(() => {
                ...
                for (const link of links) {
                  ...
                  if (
                    cell.toLowerCase().includes("schedule a") &&
                    /view\/print|view/i.test(link.textContent ?? "")
                  ) {
              ...
              const schedALocator = page
                .locator(
                  "td:has-text('Schedule A') + td a, td:has-text('Schedule A') a",
                )
                .first();
              await Promise.all([
                page.waitForNavigation({ timeout: 15000 }),
          ...
          if (!contribRows.length && totalAmt > 0) {
            contribRows = [
              { name: "", occupation: "", employer: "", amount: totalAmt },
            ];
          }

          let anyMatch = false;
          for (const cr of contribRows) {
            const bucket = classifyContrib(
              cr.name,
              cr.occupation,
              cr.employer,
              cr.amount,
          ...
          try {
            ...
            for (let p = 1; p < pageNum; p++) {
              const nextPageLocator = page
                .locator(`a[href*='doPostBack']:has-text('${p + 1}')`)
                .first();
              await Promise.all([
        ...
        await Promise.all([
          page.waitForNavigation({ timeout: 10000 }),
          page
            .locator(`a[href*='doPostBack']:has-text('${pageNum + 1}')`)
            .first()
            .click(),
        ]);
        await sleep(300);
      ...
      console.log(
        `[ks-cfr] ${office.name} done — pages=${pageNum} processed=${processed} matched=${matched.size}`,
      );
    }
  } finally {
    await browser.close();
  ...
  console.log(
    `[ks-cfr] processed=${processed} candidates_matched=${matched.size} rows_to_upsert=${upsertRows.length} total_contributions=$${totalContribs.toFixed(2)}`,
  );
  ...
  console.log(
    `[ks-cfr] complete candidates_matched=${matched.size} rows_upserted=${upserted}`,
  );
}

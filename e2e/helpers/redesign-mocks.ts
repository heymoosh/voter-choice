 ...
 * agree/disagree/pass step that now sits between the workspace and the
 * standing (polis) report. With the [P1] declutter the "see where you
 * stand" teaser was removed; the only remaining entry is the `.all-done`
 * completion link, which appears in the center (rep) column once every
 * seat has a verdict. Verdict each scorecard row in turn (selecting the
 * row first makes this deterministic across the auto-advance and works on
 * both desktop inline cards and the mobile center overlay), then open a
 * seat so the center column is visible and follow the link. On mobile the
 * center is a tap-to-open overlay that closes after each verdict, so the
 * final row click is what surfaces `.all-done`.
 *

...
- Conductor auto-merge policy: after self-vet passes and CI is green, auto-merge (server-side `gh pr merge --auto --squash`) is the DEFAULT for backend, copy, config, and data changes that merely render in the FE. HOLD only for a genuine FE design-experience change or a genuine judgment call — open those as a held draft PR with a contact sheet, for Muxin to review.

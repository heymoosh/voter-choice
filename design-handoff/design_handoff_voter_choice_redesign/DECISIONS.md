...
## Session 4 — Results detail: funding expansion + voting record (Muxin review)

Built into canvas **Section 2** as three added artboards (`res-funding`,
`res-votes`, `res-allvotes`) alongside `res-main`. Files: `screens-results.jsx`
+ `screens.css`. These **EVOLVE shipped components** — they are not new screens,
and they are **not "reused verbatim"** (the earlier handoff wording). The actual
review decisions live here.

### Decided / built
- **Funding = progressive disclosure, expanded view now defined.** "Funders &
  influence ▾" opens `FunderPanel`: total raised, small/large/PAC mix + legend,
  and the **industry breakdown** (ranked, $ + %).
  - **PAC honesty (Muxin):** **never invent named issue-PACs.** Name a PAC only
    when attributable to a public agenda (`donorCoalition[].isIssuePAC`). When
    none are, show the honest note ("About $X (Y%) came from PACs we can't yet
    attribute…") and point to the industry breakdown. The **PAC definition is a
    tooltip** on the "PACs" legend term, not body copy.
  - **Comparison is chamber-median**, not challenger-based ("≈3× the median House
    campaign") — you can't know a challenger before one is selected to compare.
  - **Wire in:** evolve `FunderBars` + the RepCard money-trail disclosure
    (`redesign2-card.jsx` → `RepCard.tsx`): add the legend tooltip, drop any
    invented-PAC rendering, add the no-attributable-PAC note path. Data:
    `/api/donors` `donorCoalition`/`fundingMix`/`totalRaised` + `isIssuePAC`;
    **new** chamber-median aggregate (see new-fields table in README §8.1).
- **"Select a vote" → per-issue drilldown.** Tapping an issue opens
  `VoteDrilldown` (compact `VoteCard`s): bill, YEA/NAY, ✓ with / ✗ against you,
  one-line summary, date + source. The issue list stays intact above; the detail
  is a sibling panel below (one issue open at a time).
  - **Wire in:** evolve `AlignmentScoreBanner` + `AlignmentDrilldown`
    (`RepCard.tsx`); driven by `alignmentEntry.scores[].contributingVotes[]`.
- **"See all votes" → full record sheet.** `AllVotesSheet` over the dimmed
  surface: filters (All / With you / Against you / per issue), grouped by issue,
  each row links to the official roll-call. It's a lot — never shown by default.
  - **Wire in:** evolve the shipped `AllVotesPanel`.
- **Click a vote → bill detail.** A row expands to a plain-language summary of
  the bill, the roll-call **tally**, its **status**, and how this rep voted.
  - **Wire in:** `AllVotesPanel` row → detail panel; needs the new bill fields.

### New fields, states & open questions
See **README §8.1** for the new-fields table (`vote.what`/`tally`/`status`,
`funding.chamberMedian`, `isIssuePAC` gating, etc.), the state/interaction rules
(PAC tooltip on touch, donor-unavailable path, see-all filters/sort, bill-detail
accordion), and the open product/eng questions. Mobile reflow for these surfaces
is in **RESPONSIVE.md §2**.

### Card footer + the component direction (review-card default locked)
- **Default review card = the full card** (not a slimmer variant). A slim
  candidate-card-shell alternative was explored and **rejected** — the review
  surface keeps attendance, the funding glance, and full evidence-on-demand.
- **Shared "evidence row"** — `See all votes → · Funders & influence ▾` now sits
  as one row above the verdict block, replacing the scattered see-all (was in the
  alignment band) and the in-glance funders toggle. In the design it's a single
  shared class (`.card-evidence`) used by the review card AND the candidate card.
- **Build these as shared components, kept in sync** (the whole point): card
  shell, blind header/reveal, alignment block, attendance line, funding
  (glance → FunderBars), evidence row, verdict block, provenance badge — composed
  into ONE card with a `mode` (`review` = dual verdict + disclosures · `compare`
  = single CTA). The review card and the candidate/compare card are the same
  component in two modes; don't fork them (that's what caused the earlier
  "close-but-not-identical" drift).

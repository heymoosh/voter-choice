...
function CanvasApp() {
  return (
    <DesignCanvas>
      ...
      <DCSection id="delegation" title="11 · Delegation overview → drill-down — the Keystone follow-up (NEW)" subtitle="Resolves the §8 parity gap Code flagged. The CandidateParity 3-card overview becomes the ENTRY POINT: every seat scored at once, then click any card to open the EXISTING deep single-seat review. The right rail stays as in-context lateral nav. Muxin's direction, 2026-07-07.">
        <DCPostIt top={150} left={64} width={206}>RESOLVED (Muxin): the 3-card overview is the entry point; clicking a card opens the existing deep single-seat review <b>unchanged</b> — no new deep-dive design. Two things layered on: a “← All seats” back in the context strip, and the seat-strip rail STAYS for lateral nav (jump seat→seat without returning to the overview). Non-2026 seats stay greyed + excluded (97eda1e0). Verdicts ride along — decide a seat and both the overview and rail update. Senate is pre-decided (Keep) to show that state.</DCPostIt>
        <DCArtboard id="dg-overview" label="Entry point · every seat scored — the overview" width={1180} height={784}><DelegationOverview verdicts={{ senate: "keep" }} /></DCArtboard>
        <DCPostIt top={150} left={1292} width={206}>INTERACTIVE — this is the resolved flow. Click a seat card → it opens the deep review. “← All seats” returns you. Inside a seat, the rail on the right switches you between seats laterally, and the Keep / Replace buttons record a verdict that shows back on the overview. Try it.</DCPostIt>
        <DCArtboard id="dg-flow" label="★ Interactive · click a card → deep view · ← All seats · rail switches seats" width={1180} height={844}><DelegationFlow /></DCArtboard>
        <DCPostIt top={150} left={2520} width={206}>THE DRILL-DOWN TARGET: the deep view opened from a card is the SAME single-seat review surface the results screen already uses (.rcard + progress rail) — reused verbatim, not redesigned. The only additions are the “← All seats” back and the rail behaving as lateral nav. Shown here opened on the House seat.</DCPostIt>
        <DCArtboard id="dg-deep" label="Drill-down · the existing deep single-seat review (opened from the overview)" width={1180} height={844}><DelegationFlow initialActive="house" /></DCArtboard>
      </DCSection>
